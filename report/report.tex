...
During our testing the performance has scaled positively as the size of the dataset increases, especially with the Adam solver, which matches scikit-learn's documentation\cite{sklearn:MLPClassifier}. The cost of using the Adam solver over the LBFGS is higher, increasing the fitting sixfold on a training set of 42.000 documents. It showed a slightly higher performance when applying Youden’s Index. 2 hidden layers of 100 nodes performs slightly better than 1 layer of 100 nodes, but increases the model fitting duration dramatically*. 1 layer of 25 nodes seems to perform worse with both solvers.
