...
Our starting point of data prepossessing was to understand what an Amazon review consists of. The raw data can be seen in \figurename{\ref{fig:unclean_data}}.
...
When you consider ethics in NLP, you need to first look at the bias the can occur. A review might say “Great product, used it for a week!”. This is perceived as helpful by some customers, since the reviewer expresses their opinion on the product with sentiment information. But some other consumer might not find it helpful due to the lack of more details or an analysis of the product. This leads to reviews being biased towards the consumers uses, and needs.

Another thing that frequently occurs in reviews is herd behaviour. If we have a product, then depending on the first review given to it, other reviews might tend to follow. If the first review is very positive, other reviews might have a tendency to also grade the product higher. This leads to reviews being untrustworthy.
\cite{Yves:2018dg}.
%------------------------------------------------
